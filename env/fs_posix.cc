...
class PosixFileSystem : public FileSystem {
 public:
  ...
  FileOptions OptimizeForCompactionTableRead(
      const FileOptions& env_options,
      const ImmutableDBOptions& db_options) const override {
    FileOptions fo = FileOptions(env_options);
#ifdef OS_LINUX
    // To fix https://github.com/facebook/rocksdb/issues/12038
    if (!env_options.use_direct_reads &&
        env_options.compaction_readahead_size > 0) {
      size_t system_limit =
          GetCompactionReadaheadSizeSystemLimit(db_options.db_paths);
      if (system_limit > 0 &&
          env_options.compaction_readahead_size > system_limit) {
