...
Reader::Reader(std::shared_ptr<Logger> info_log,
    ...
    : info_log_(info_log),
      ...
      predecessor_wal_info_type_record_read_(false),
      predecessor_log_number_(0),
      predecessor_size_bytes_(0),
...
bool Reader::ReadRecord(
    Slice* record, std::string* scratch, WALRecoveryMode wal_recovery_mode,
    uint64_t* record_checksum, const bool* stop_replay_for_corruption,
    const uint64_t* min_wal_number, uint64_t* prev_log_number,
    SequenceNumber* prev_log_last_seqno_recorded, uint64_t* prev_log_size) {
  scratch->clear();
  record->clear();
  ...
  while (true) {
    ...
    switch (record_type) {
      ...
      case kRecyclePredecessorWALInfoType: {
        ...
        scratch->clear();
        last_record_offset_ = prospective_record_offset;
        PredecessorWALInfoRecord predecessor_wal_info_record(
            0 /* log_number */, 0 /* size_bytes */,
            kMaxSequenceNumber /* last_seqno_recorded */);
        Status s = predecessor_wal_info_record.DecodeFrom(&fragment);
        if (!s.ok()) {
          ReportCorruption(fragment.size(),
          ...
          predecessor_last_seqno_recorded_ =
              predecessor_wal_info_record.GetLastSeqnoRecorded();
          ...
          if (wal_recovery_mode != WALRecoveryMode::kSkipAnyCorruptedRecords &&
            ...
            if (*prev_log_number == 0) {
              ...
              if (predecessor_size_bytes_ != *prev_log_size) {
                ReportCorruption(fragment.size(), "Hole in size of same seqno");
                return false;
              }
            }
          }
...
bool FragmentBufferedReader::ReadRecord(
    Slice* record, std::string* scratch, WALRecoveryMode wal_recovery_mode,
    uint64_t* /* checksum */, const bool* stop_replay_for_corruption,
    const uint64_t* min_wal_number, uint64_t* prev_log_number,
    SequenceNumber* prev_log_last_seqno_recorded, uint64_t* prev_log_size) {
